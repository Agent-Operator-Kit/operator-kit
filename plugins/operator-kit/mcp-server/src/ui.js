...
function projectSidebar() {
  const visibleProjects = preferences.allProjects ? projects : projects.filter(project => project.root === state.project.root);
  const entries = visibleProjects.length ? visibleProjects : [{ ...state.project, available: true, summary: { attention: state.summary.attention, active: state.summary.recordedActiveTasks } }];
  return `<aside class="projects" id="project-sidebar" aria-label="${h('Registered projects')}"><div class="sidebar-heading"><span class="sidebar-label">${h('Workspace')}</span><button id="sidebar-toggle" class="icon-button" aria-expanded="${!sidebarCollapsed}" aria-controls="project-list" aria-label="${h(sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar')}" title="${h(sidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar')}"><svg viewBox="0 0 16 16" aria-hidden="true"><rect x="2" y="2" width="12" height="12" rx="2"/><path d="M6 2v12"/></svg></button></div><div id="project-list">${entries.map(project => `<button class="project-row ${project.root === state.project.root ? 'selected' : ''}" data-project="${escape(project.root)}" data-available="${project.available}" aria-label="${escape(project.name)}" title="${escape(project.name)}" aria-pressed="${project.root === state.project.root}"><span class="project-avatar" aria-hidden="true">${escape(project.name.slice(0,2).toUpperCase())}</span><span class="project-copy"><strong>${escape(project.name)}</strong>${project.available ? `<small>${h('Need attention')}: ${number(project.summary.attention)}</small>` : `<small>${h('Unavailable')}</small>`}</span></button>`).join('')}</div><div class="sidebar-extra"><p class="muted sidebar-note">${h('Only registered local projects are listed.')}</p>${projectsError ? `<p class="warning">${h('Project list unavailable')}: ${escape(projectsError)}</p>` : ''}<small id="projects-time">${h('Project list last checked')} ${stamp(projectsAt)}</small><details data-record="add-project"><summary>${h('Add project')}</summary><form id="add-project"><label>${h('Project root')}<input id="project-root-input" name="projectRoot" required placeholder="${h('Absolute folder containing operator.config.env')}" autocomplete="off"></label><button type="submit" class="secondary">${h('Add')}</button></form></details></div></aside>`;
}

function render() {
  const y = window.scrollY, focused = document.activeElement;
  const featureScroll = document.querySelector('.feature-list')?.scrollTop || 0;
  ...
  document.querySelector('.feature-list').scrollTop = featureScroll;
