...
function displayModeControl() {
  document.querySelectorAll('[data-display-mode]').forEach(button => {
    const supported = !availableDisplayModes || availableDisplayModes.includes(button.dataset.displayMode);
  ...
  document.querySelectorAll('[data-expand-feature]').forEach(button => {
    button.disabled = changingDisplayMode || (availableDisplayModes && !availableDisplayModes.includes('fullscreen'));
  });
}
...
async function requestDisplayMode(mode) {
  ...
  try {
    ...
    if (result.mode && result.mode !== mode) say(mode === 'inline' ? 'The host kept the console expanded.' : 'The host did not expand the console.');
...
function detail(feature) {
  ...
  return `<section class="detail panel"><div class="detail-head"><div><span class="eyebrow">${escape(feature.id)} · ${escape(feature.branch || 'no branch')}</span><h2>${escape(feature.title)}</h2></div><div class="detail-controls"><span class="status">${escape(feature.status)}</span><button class="secondary" data-display-mode="fullscreen">View full screen</button></div></div><p class="detail-meta">${feature.boundChats.length} bound Codex task(s) · ${escape(feature.worktree || 'No worktree recorded')}</p><nav aria-label="Feature detail">${tabs.map(tab => `<button data-tab="${tab}" aria-pressed="${tab === currentTab}">${tab[0].toUpperCase() + tab.slice(1)}</button>`).join('')}</nav>${content}<div class="actions">${feature.boundChats.map((chat, i) => `<button class="secondary" data-chat="${escape(chat.id)}">Open Codex task${feature.boundChats.length > 1 ? ` ${i + 1}` : ''}</button>`).join('') || '<span class="muted">No Codex conversation bound.</span>'}</div><p class="safety">Open task asks the host conversation to navigate. It does not start work.</p></section>`;
}
function render() {
  ...
  document.querySelectorAll('[data-expand-feature]').forEach(button => button.onclick = () => { selectedId = button.dataset.expandFeature; selectedTask = null; render(); saveView(); requestDisplayMode('fullscreen'); });
