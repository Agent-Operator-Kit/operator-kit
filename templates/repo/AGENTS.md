...
When operating from Cursor, read `.cursor/rules/operator-workflow.mdc`. Use `.cursor/skills/operator/SKILL.md` for day-to-day execution, `.cursor/skills/operator-planner/SKILL.md` for roadmap/backlog planning, `.cursor/skills/operator-feedback/SKILL.md` for feedback intake, `.cursor/skills/design-agent/SKILL.md` for UX/design work, `.cursor/skills/incubation/SKILL.md` for idea incubation and promotion readiness, and `.cursor/skills/operator-workflow/SKILL.md` for setup, status, dispatch, collection, and repair workflows. In Cursor-first environments without Codex, use Cursor IDE as the operator lane, Cursor CLI as a local worker lane when available, and Claude Code as an optional scoped worker.
...
mode, UX/design-system work to design-agent mode, and idea incubation to
incubation mode unless the user asks for execution.
