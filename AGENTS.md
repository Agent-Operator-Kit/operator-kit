...
This repository is the Operator Kit source distribution inside the scoped
project root `$HOME/projects/operator-kit`. Its self-hosting configuration is
kept beside `code/` at `$HOME/projects/operator-kit/operator.config.env`, and
all generated Operator state lives at `$HOME/projects/operator-kit/operator`.
Run project-local Operator commands with
`OPERATOR_CONFIG="$HOME/projects/operator-kit/operator.config.env"`. Keeping
the config outside `code/app` preserves the source-package invariant enforced
by `tests/smoke/v3-host-adapters.sh`.
...
- In V5.2, use one local dependency graph per feature session to identify ready,
...
- V5.2 requires no signing authority, Keychain, proof broker, trusted host,
  lease, or fence.
...
- `OPERATOR_DIR/features/<FS-id-slug>/` stores V4 feature-session state, memory, lane-instance packets, handoffs, merge plans, conflicts, and working files when feature sessions are enabled.
...
- In V4, never spawn or dispatch a lane instance until the operator has checked
  branch, worktree, file, contract, and shared-resource conflicts.
