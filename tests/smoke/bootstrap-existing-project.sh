...
grep -q 'OPERATOR_KIT_VERSION="5.1"' "$tmp_root/code/app/operator.config.env"
grep -q 'codex --sandbox workspace-write' "$tmp_root/code/app/operator.config.env"
grep -q 'claude --model fable --permission-mode dontAsk' "$tmp_root/code/app/operator.config.env"
...
for v5_script in operator-role-map.sh operator-graph.sh operator-v5-1-migrate.sh; do
...
test ! -e "$tmp_root/code/app/schemas/operator-v5"
